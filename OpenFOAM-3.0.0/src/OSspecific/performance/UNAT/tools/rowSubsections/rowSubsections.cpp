...
void SWFoam::RowSubsections::decomposeFaces()
{
    ...
    label avgSegLength = label( (scalar)_nFace/(scalar)_segNum );
    ...
    for(int i = 1; i < _segNum; i++)
    {
        ...
        {
            printf("\n***Error: segment %d start %d exceed the face number %d!\n\n",i, tmpStart, _nFace);
            exit(1);
        }
        if( _lPtr[tmpStart] == _nCell-1)
        {
        ...
        segStart[i] = tmpStart;
		avgSegLength = label((scalar)(_nFace-tmpStart)/(scalar)(_segNum-i));
    ...
    for(int i = 0; i < _segNum; i++ )
    {
		...
		if(i==0) _subsections[i][0].cellStart = 0;
		else     _subsections[i][0].cellStart = _lPtr[segStart[i]-1]+1;
        ...
        for(int j = 1; j < _secNumInSeg; j++)
        {
            ...
            {
                printf("\n***Error: subsection face starts exceed rows in segment %d %d, %d, %d, %d, %d!\n\n", i,tmpStart, segStart[i+1]-1, j, _lPtr[tmpStart], _secNumInSeg);
                //exit(1);
            }
            while( _lPtr[tmpStart]  == _lPtr[tmpStart-1] )  tmpStart++;
            _subsections[i][j].faceStart  = tmpStart;
            _subsections[i][j-1].nFaces
				= tmpStart - _subsections[i][j-1].faceStart;
			_subsections[i][j].cellStart  = _lPtr[tmpStart-1]+1;
			_subsections[i][j-1].nCells
				= _lPtr[tmpStart-1]-_subsections[i][j-1].cellStart+1;
        }
        _subsections[i][_secNumInSeg-1].nFaces
			= segStart[i+1] - _subsections[i][_secNumInSeg - 1].faceStart;
		if(i==_segNum-1) 
		{
			_subsections[i][_secNumInSeg-1].nCells
				= _nCell-_subsections[i][_secNumInSeg-1].cellStart;
		} else
		{
			_subsections[i][_secNumInSeg-1].nCells
				=_lPtr[segStart[i+1]-1]-_subsections[i][_secNumInSeg-1].cellStart+1;
		}
    }
