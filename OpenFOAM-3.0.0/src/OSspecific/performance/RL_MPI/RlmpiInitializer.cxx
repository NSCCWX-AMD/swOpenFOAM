...
void generate_register_transform_table(int(*dst_list)[64],
        int(*sendN)[64], Table *table) {
    int i, j;
    for (i = 0; i < 64; i++) {
        table[i].nGETC = 0;
        table[i].nPUTR = 0;
        table[i].nGETR_PUTC = 0;
    }
    ...
    for (dstId = 0; dstId < 64; dstId++) {
        ...
        for (srcId = 0; srcId < 64; srcId++) {
            ...
            if (dst == dstId) {
                ...
                table[dstId].nGETC += ns;
            }
        }
        //        DISP(table[dstId].nGETC);
    }
...
void RlmpiInitializer::generate_data_un_col_row(const vector<vector < int8LDM> > &not_col_row_dst) {
...
void RlmpiInitializer::generate_table() {
    for (int myId = 0; myId < nThread; myId++) {
        //        table[myId].maxOP = 0;
        table[myId].nGETC = 0;
        table[myId].nGETR_PUTC = 0;
        table[myId].nPUTR = 0;
    ...
    generate_register_transform_table(dst_list, sendN_list, table);
...
void RlmpiInitializer::reorder_packages() {
    ...
    for (int myId = 0; myId < nThread; myId++) {
        ...
        for (int n = 0; n < table[myId].nPUTR; n++) {
            for (map<int, vector<Pack> >::iterator it = packages[myId].begin();
                    it != packages[myId].end(); it++) {
...
void RlmpiInitializer::reorder_packages2() {
    ...
    for (int myId = 0; myId < nThread; myId++) {
        ...
        //        cout << "res " << res_packages[myId].size() << endl;
    }
...
void RlmpiInitializer::transpose_matrix(vector<Pack> &pack) {
    vector<Pack> res(pack.size());
    for (int i = 0; i < 7; i++) {
        for (int j = 0; j < 7; j++) {
            res[j * 7 + i] = pack[i * 7 + j];
        }
    }
    pack = res;
}
...
void RlmpiInitializer::generate_schedule() {
...
void RlmpiInitializer::generate_data_un_col_row(const vector<vector <int8LDM> > &src2dst, const vector<vector <int8LDM> > &nData) {
    for (int myId = 0; myId < nThread; myId++) {
        packages[myId].clear();
        for (int s = 0; s < src2dst[myId].size(); s++) {
            Pack pack;
            ...
            //            pack.s_col = COL(pack.src_id);
            //            pack.s_row = ROW(pack.src_id);
            //            pack.nData = nData[myId][s];

            pack.data[0] = sReal(myId) + sReal(s) / 1e2;
            pack.data[1] = sReal(myId) + sReal(s) / 1e3;
            pack.data[2] = sReal(myId) + sReal(s) / 1e4;
...
void RlmpiInitializer::init(const vector<vector<ThreadID> > &sendDstLists) {
    ...
    generate_data_un_col_row(not_col_row_dst, not_col_row_Ndata);
    generate_table();
    reorder_packages();
    generate_schedule();
...
void RlmpiInitializer::copyinfo(Schedule *schedule_data) {
    ...
    for (int myId = 0; myId < 64; myId++) {
        ...
        int total_send = table[myId].nPUTR + table[myId].nPutcSameCol + table[myId].nPutrSameRow;
        int total_recv = table[myId].nGETC + table[myId].nGetcSameCol + table[myId].nGetrSameRow;

        if (total_recv != total_send) {
            //            DISP(table[myId].nPUTR);
            //            DISP(table[myId].nPutcSameCol);
            //            DISP(table[myId].nPutrSameRow);
            //
            //            DISP(table[myId].nGETC);
            //            DISP(table[myId].nGetcSameCol);
            //            DISP(table[myId].nGetrSameRow);
            //            DISP2(total_recv, total_send);
            //            abort();
        }
