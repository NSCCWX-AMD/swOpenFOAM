...
#ifdef SW_SLAVE
#include "swNegSumDiag/swNegSumDiag.h"
#endif
...
#ifdef SW_SLAVE
	const LABEL *lPtr = (LABEL*)(&l.begin()[0]);
	const LABEL *uPtr = (LABEL*)(&u.begin()[0]);
	const SCALAR *lowerPtr = (SCALAR*)(&Lower.begin()[0]);
	const SCALAR *upperPtr = (SCALAR*)(&Upper.begin()[0]);
	SCALAR *diagPtr = (SCALAR*)(&Diag.begin()[0]);
	negSumDiag_host(lowerPtr,upperPtr,diagPtr,lPtr,uPtr,Diag.size(),l.size());
#else
    for (label face=0; face<l.size(); face++)
    {
        Diag[l[face]] -= Lower[face];
