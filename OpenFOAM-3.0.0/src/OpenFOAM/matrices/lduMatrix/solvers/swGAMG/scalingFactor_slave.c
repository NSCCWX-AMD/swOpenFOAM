...
void scalingFactor_slave(MVM_ParametersPack* MVM_ParametersPack_hostPtr)
{
    ...
    for(i=sizePerCycle; i<=lenLocal; i+=sizePerCycle)
    {
        ...
        LABEL j, nsimd = (sizePerCycle / 4) * 4;
        doublev4 a1, a2, a3, a4;
        for(j=0; j<nsimd; j+=4)
        {
            simd_load(a1, &(A1_slavePtr[j]));
            simd_load(a2, &(A2_slavePtr[j]));
            simd_load(a3, &(A3_slavePtr[j]));
            simd_load(a4, &(A4_slavePtr[j]));

            b1 += a2 * a1;
            b2 += a3 * a1;
            a1 += (a2 - a3) / a4;
            simd_store(a1, &(A1_slavePtr[j]));
        }
        for(j=nsimd; j<sizePerCycle; j++)
        {
            k1Num_slave    += A2_slavePtr[j] * A1_slavePtr[j];
            k2Denom_slave  += A3_slavePtr[j] * A1_slavePtr[j];
            A1_slavePtr[j] += (A2_slavePtr[j] - A3_slavePtr[j]) / A4_slavePtr[j];
        }
    ...
    SCALAR *b1Ptr = (SCALAR*)&b1;
    SCALAR *b2Ptr = (SCALAR*)&b2;
    k1Num_slave   += b1Ptr[0] + b1Ptr[1] +
                     b1Ptr[2] + b1Ptr[3];
    k2Denom_slave += b2Ptr[0] + b2Ptr[1] +
                     b2Ptr[2] + b2Ptr[3];
